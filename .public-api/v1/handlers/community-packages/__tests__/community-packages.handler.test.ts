...
describe('CommunityPackages Handler', () => {
  ...
  describe('installPackage', () => {
    ...
    it('should throw BadRequestError when name is missing', async () => {
      ...
      mockLifecycle.install.mockRejectedValue(new BadRequestError(RESPONSE_ERROR_MESSAGES.PACKAGE_NAME_NOT_PROVIDED));
    ...
    it('should throw BadRequestError when package is already installed', async () => {
      ...
      mockLifecycle.install.mockRejectedValue(new BadRequestError('Package "n8n-nodes-test" is already installed'));
  ...
  describe('getInstalledPackages', () => {
    it('should return installed packages', async () => {
      ...
      await handler.getInstalledPackages[handler.getInstalledPackages.length - 1](req, mockResponse);

      expect(mockResponse.json).toHaveBeenCalledWith(mapToCommunityPackageList([mockInstalledPackage]));
    });
  ...
  describe('updatePackage', () => {
    ...
    it('should throw NotFoundError when package is not installed', async () => {
      ...
      mockLifecycle.update.mockRejectedValue(new NotFoundError(RESPONSE_ERROR_MESSAGES.PACKAGE_NOT_INSTALLED));
  ...
  describe('uninstallPackage', () => {
    ...
    it('should throw NotFoundError when package is not installed', async () => {
      ...
      mockLifecycle.uninstall.mockRejectedValue(new NotFoundError(RESPONSE_ERROR_MESSAGES.PACKAGE_NOT_INSTALLED));
