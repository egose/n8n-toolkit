...
describe('CredentialsService', () => {
  ...
  describe('toJsonSchema', () => {
    it('should create separate conditionals for different values of the same dependant field', () => {
      ...
      expect(pemKeyCondition.then?.allOf.some((req: any) => req.required?.includes('privateKey'))).toBe(true);
      expect(pemKeyCondition.then?.allOf.some((req: any) => req.required?.includes('publicKey'))).toBe(true);
      expect(pemKeyCondition.then?.allOf.some((req: any) => req.required?.includes('secret'))).toBe(false);
    });
    ...
    it('should add "false" displayOptions.show dependant value as allof condition', () => {
      ...
      expect(condition.else?.allOf.some((notReq: any) => notReq.not?.required?.includes('field3'))).toBe(true);
    });
  });
  ...
  describe('updateCredential', () => {
    ...
    describe('external secrets', () => {
      ...
      it('should throw error when user without permission tries to add external secret expression', async () => {
        ...
        jest.mocked(credentialsService.decrypt).mockResolvedValue({ apiKey: 'regular-secret' }); // pragma: allowlist secret
      ...
      it('should throw error when user without permission tries to modify existing external secret expression', async () => {
        ...
        jest.mocked(credentialsService.decrypt).mockResolvedValue({ apiKey: '{{ $secrets.vault.oldKey }}' });
      ...
      it('should throw error when external secret store referenced in expression is not shared with current project', async () => {
        ...
        const secretProviderKey = 'vault'; // pragma: allowlist secret
        const secretExpression = `={{ $secrets.${secretProviderKey}.myKey }}`; // pragma: allowlist secret

        credentialsRepository.findOne = jest.fn().mockResolvedValue(existingCredential);
        credentialsRepository.update = jest.fn().mockResolvedValue(undefined);
        jest.mocked(credentialsService.decrypt).mockResolvedValue({
          apiKey: 'currentPlainTextValue', // pragma: allowlist secret
        });
        jest.mocked(mockSecretsProviderAccessCheckService.isProviderAvailableInProject).mockResolvedValue(false);
        ...
        ).rejects.toThrow('The secret provider "vault" used in "apiKey" does not exist in this project');
      });
      ...
      it('should allow updates when no external secret expression is being changed', async () => {
        ...
        jest.mocked(credentialsService.decrypt).mockResolvedValue({ apiKey: '{{ $secrets.vault.myKey }}' });
      ...
      it('should allow updates of non-secret data without specific external-secret permission', async () => {
        ...
        jest.mocked(credentialsService.decrypt).mockResolvedValue({ apiKey: 'regular-key' }); // pragma: allowlist secret

        credentialsRepository.update = jest.fn().mockResolvedValue(undefined);

        await updateCredential(existingCredential, memberUser, {
          data: { apiKey: 'another-regular-key' }, // pragma: allowlist secret
        });
      });
      ...
      it('should allow user with permission to add external secret expression', async () => {
        ...
        credentialsRepository.update = jest.fn().mockResolvedValue(undefined);
        jest.mocked(credentialsService.decrypt).mockResolvedValue({ apiKey: 'regular-key' }); // pragma: allowlist secret
        jest.mocked(mockSecretsProviderAccessCheckService.isProviderAvailableInProject).mockResolvedValue(true);
        credentialsRepository.update = jest.fn().mockResolvedValue(undefined);
