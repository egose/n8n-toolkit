...
describe('DataTable Handler', () => {
  ...
  describe('getDataTableRows', () => {
    it('should retrieve rows successfully', async () => {
      ...
      expect(mockDataTableService.getManyRowsAndCount).toHaveBeenCalledWith(dataTableId, projectId, {
        skip: 0,
        take: 100,
        filter: undefined,
        sortBy: undefined,
    ...
    it('should handle filter, sortBy, and search parameters', async () => {
      // Arrange
      const filterStr = '{"type":"and","filters":[{"columnName":"status","condition":"eq","value":"active"}]}';
      const req = {
        params: { dataTableId },
        query: {
          offset: '10',
    ...
    it('should throw NotFoundError when data table not found', async () => {
      ...
      mockDataTableService.getProjectIdForDataTable.mockRejectedValue(new DataTableNotFoundError(dataTableId));
  ...
  describe('Security - Cross-Project Access', () => {
    ...
    it('should throw NotFoundError when trying to get rows from another users data table', async () => {
      ...
      mockDataTableService.getProjectIdForDataTable.mockRejectedValue(new DataTableNotFoundError(otherUserDataTableId));
      ...
      // Assert
      expect(mockDataTableService.getProjectIdForDataTable).toHaveBeenCalledWith(otherUserDataTableId);
      expect(caught).toBeInstanceOf(NotFoundError);
      expect(caught).toMatchObject({
    ...
    it('should throw NotFoundError when trying to insert rows into another users data table', async () => {
      ...
      mockDataTableService.insertRows.mockRejectedValue(new DataTableNotFoundError(otherUserDataTableId));
    ...
    it('should throw NotFoundError when trying to update rows in another users data table', async () => {
      ...
      mockDataTableService.updateRows.mockRejectedValue(new DataTableNotFoundError(otherUserDataTableId));
    ...
    it('should throw NotFoundError when trying to upsert row in another users data table', async () => {
      ...
      mockDataTableService.upsertRow.mockRejectedValue(new DataTableNotFoundError(otherUserDataTableId));
    ...
    it('should throw NotFoundError when trying to delete rows from another users data table', async () => {
      ...
      mockDataTableService.deleteRows.mockRejectedValue(new DataTableNotFoundError(otherUserDataTableId));
