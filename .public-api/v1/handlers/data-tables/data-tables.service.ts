...
export async function getDataTableListFilter(
  ...
  const personalProject = await Container.get(ProjectRepository).getPersonalProjectForUserOrFail(userId);
  ...
  const teamProjectIds = projectRelations.filter((rel) => rel.project.type === 'team').map((rel) => rel.projectId);
...
export async function resolveProjectIdForCreate(user: User, requestedProjectId?: string): Promise<string> {
  if (!requestedProjectId) {
  ...
  const project = await Container.get(ProjectService).getProjectWithScope(user, requestedProjectId, [
    'dataTable:create',
  ]);
