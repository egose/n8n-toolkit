...
jest.spyOn(middlewares, 'publicApiScope').mockImplementation((scope: ApiKeyScope) => {
  return Object.assign((req: unknown, res: unknown, next: unknown) => createMockMiddleware(req, res, next), {
    __apiKeyScope: scope,
  }) as middlewares.ScopeTaggedMiddleware;
});
jest
  .spyOn(middlewares, 'apiKeyHasScopeWithGlobalScopeFallback')
  .mockImplementation((config: { scope: ApiKeyScope } | { apiKeyScope: ApiKeyScope; globalScope: unknown }) => {
...
describe('buildDiscoverResponse', () => {
  ...
  it('should include requestSchema when includeSchemas is true', async () => {
    ...
    const createEndpoint = result.resources.credential?.endpoints.find((e) => e.operationId === 'createCredential');
  ...
  it('should resolve nested $ref in schemas recursively', async () => {
    ...
    const createEndpoint = result.resources.workflow?.endpoints.find((e) => e.operationId === 'createWorkflow');
    expect(createEndpoint).toBeDefined();
    expect(createEndpoint?.requestSchema).toBeDefined();
...
describe('resource and operation filtering', () => {
  it('should filter to a single resource when resource option is set', async () => {
    const result = await buildDiscoverResponse(['tag:list', 'tag:create', 'workflow:read'] as ApiKeyScope[], {
      resource: 'tags',
    });
  ...
  it('should apply both resource and operation filters together', async () => {
    ...
    expect(result.resources.workflow.endpoints.every((e) => e.operationId === 'createWorkflow')).toBe(true);
  });
});
