 ...
 */
function extractRequestSchema(operation: Record<string, unknown>): Record<string, unknown> | undefined {
...
async function _parseEndpointsFromSpec(): Promise<EndpointInfo[]> {
  ...
  for (const [pathKey, pathValue] of Object.entries(spec.paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      const scope = Array.isArray(middlewareChain) ? extractScopeFromHandler(middlewareChain) : null;
