...
function isRedactableExecution(execution: IExecutionBase): execution is IExecutionBase & RedactableExecution {
...
const executionHandlers: ExecutionHandlers = {
  deleteExecution: [
    ...
    async (req, res) => {
      ...
      const execution = await Container.get(ExecutionPersistence).getExecutionInWorkflowsForPublicApi(
        id,
        sharedWorkflowsIds,
        false,
      );
  ...
  getExecution: [
    ...
    async (req, res) => {
      ...
      if (includeData && isRedactableExecution(execution)) {
        ...
        const redactExecutionData = redactQuery.success ? redactQuery.data.redactExecutionData : undefined;
  ...
  getExecutions: [
    ...
    async (req, res) => {
      ...
      const executions = await Container.get(ExecutionRepository).getExecutionsForPublicApi(filters);

      const newLastId = !executions.length ? '0' : executions.slice(-1)[0].id;

      filters.lastId = newLastId;

      const count = await Container.get(ExecutionRepository).getExecutionsCountForPublicApi(filters);
      ...
      if (includeData) {
        ...
        await Container.get(ExecutionRedactionServiceProxy).processExecutions(redactableExecutions, {
          user: req.user,
          redactExecutionData,
          ipAddress: req.ip ?? '',
          userAgent: req.headers['user-agent'] ?? '',
  ...
  retryExecution: [
    ...
    async (req, res) => {
      ...
      try {
        const retriedExecution = await Container.get(ExecutionService).retry(req, sharedWorkflowsIds);
