...
export async function updateExecutionTags(executionId: string, newTagIds: string[]): Promise<AnnotationTagEntity[]> {
  ...
  return await dbManager.transaction(async (transactionManager) => {
    ...
    await transactionManager.upsert(ExecutionAnnotation, { execution: { id: executionId } }, ['execution']);
