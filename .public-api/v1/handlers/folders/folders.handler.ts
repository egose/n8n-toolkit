...
import { apiKeyHasScopeWithGlobalScopeFallback, isLicensed } from '../../shared/middlewares/global.middleware';
...
const folderHandlers: FolderHandlers = {
  ...
  getFolders: [
    ...
    async (req, res) => {
      ...
      const [data, count] = await Container.get(FolderService).getManyAndCount(projectId, query.data);
  ...
  updateFolder: [
    ...
    async (req, res) => {
      ...
      try {
        const folder = await Container.get(FolderService).updateFolder(req.params.folderId, projectId, payload.data);
