...
type ProjectHandlers = {
  ...
  changeUserRoleInProject: PublicAPIEndpoint<AuthenticatedRequest<{ projectId: string; userId: string }>>;
  deleteUserFromProject: PublicAPIEndpoint<AuthenticatedRequest<{ projectId: string; userId: string }>>;
};
...
const projectHandlers: ProjectHandlers = {
  ...
  updateProject: [
    ...
    async (req, res) => {
      ...
      await Container.get(ProjectController).updateProject(req, res, payload.data, req.params.projectId);
  ...
  deleteProject: [
    ...
    async (req, res) => {
      ...
      await Container.get(ProjectController).deleteProject(req, res, query.data, req.params.projectId);
  ...
  getProjectUsers: [
    ...
    async (req, res) => {
      ...
      const project = await projectService.getProjectWithScope(req.user, projectId, ['project:list']);
      if (!project) {
      ...
      const memberFields = ['id', 'email', 'firstName', 'lastName', 'createdAt', 'updatedAt'] as const;
  ...
  addUsersToProject: [
    ...
    async (req, res) => {
      ...
      await Container.get(ProjectService).addUsersToProject(req.params.projectId, payload.data.relations);
