...
import { publicApiScope, projectScope, validCursor } from '../../shared/middlewares/global.middleware';
...
const workflowHandlers: WorkflowHandlers = {
  ...
  transferWorkflow: [
    ...
    async (req, res) => {
      ...
      await Container.get(EnterpriseWorkflowService).transferWorkflow(req.user, workflowId, body.destinationProjectId);
  ...
  getWorkflow: [
    ...
    async (req, res) => {
      ...
      const workflow = await Container.get(WorkflowFinderService).findWorkflowForUser(id, req.user, ['workflow:read'], {
  ...
  getWorkflowVersion: [
    ...
    async (req, res) => {
      ...
      try {
        const version = await Container.get(WorkflowHistoryService).getVersion(req.user, workflowId, versionId, {
  ...
  getWorkflows: [
    ...
    async (req, res) => {
      ...
      if (['global:owner', 'global:admin'].includes(req.user.role.slug)) {
        if (tags) {
          const workflowIds = await Container.get(TagRepository).getWorkflowIdsViaTags(parseTagNames(tags));
        ...
        if (tags) {
          options.workflowIds = await Container.get(TagRepository).getWorkflowIdsViaTags(parseTagNames(tags));
        }
  ...
  updateWorkflow: [
    ...
    async (req, res) => {
      ...
      try {
        const updatedWorkflow = await Container.get(WorkflowService).update(req.user, updateData, id, {
  ...
  updateWorkflowTags: [
    ...
    async (req, res) => {
      ...
      const sharedWorkflow = await Container.get(WorkflowFinderService).findWorkflowForUser(id, req.user, [
        'workflow:update',
      ]);
