...
export async function getSharedWorkflowIds(user: User, scopes: Scope[], projectId?: string): Promise<string[]> {
...
export async function getSharedWorkflow(user: User, workflowId?: string): Promise<SharedWorkflow | null> {
  return await Container.get(SharedWorkflowRepository).findOne({
    ...
    relations: [...insertIf(!Container.get(GlobalConfig).tags.disabled, ['workflow.tags']), 'workflow'],
  });
}
...
export async function createWorkflow(
  ...
  await Container.get(RedactionEnforcementService).assertNewPolicyAllowed(workflow.settings?.redactionPolicy);
