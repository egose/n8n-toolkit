...
- **Credential create publication is id-only** — on the pinned n8n `2.31.2` contract used by this repo's example images, publish immediately when the hook payload includes `credential.id` and `data`, or briefly retry `dbCollections.Credentials.findOne({ where: { id } })` when only the stable id is available. Never fall back to `{name, type}`; payloads without `id` must be logged and dropped to avoid cross-publishing another credential. In practice n8n fires `credentials.create` pre-commit with `id: null`, so creates never sync alone — `workflow.upsert`/`workflow.activate` carry id-only `credentialIds` (see `src/shared/credential-refs.ts`), the subscriber reports the missing ones in the delivery response, and the publisher backfills exactly those blobs by stable id.
...
- **`SYNC_APPLY_ACTIVE_STATE` (default false)** — when true, the subscriber publishes/unpublishes the synced workflow on the target via n8n's `WorkflowService.activateWorkflow`/`deactivateWorkflow` (after ensuring a `workflow_history` version row exists), so triggers register with the target's active workflow manager. Publication failures degrade to warn-only; the event still returns applied. Requires the target owner's user record for the publish call; without resolvable `WorkflowService`/`WorkflowHistoryService` (see `N8N_CORE_PATH`) it falls back to DB-column writes with a startup warning.
...
src/
  shared/
    ...
    credential-refs.ts — id-only extraction of workflow-node credential references (capped, deduped)
