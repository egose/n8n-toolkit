...
When you want bound instance methods instead of plain API objects, use `getResource()` or `listResources()`. The full model is documented on the [N8nClient API](/api/n8n-client/) page.

## Next Steps

- Browse the [API Reference](/api/n8n-client/) for the full method surface.
- Read [Philosophy](/about/philosophy/) if you want the design rationale behind resource clients, retries, and low-level request helpers.
